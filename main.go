...
func main() {
	...
	searchParamsPreset := request.CreateSeekSearchApiParamsFromConfig(readConfig.SearchParamsPreset)

	// postData := []request.SeekSearchApiResponseData{}
	// postDetails := []request.SeekPostDetails{}
	combinedKeywords := request.CreateSearchCombinations(*readConfig.SearchKeywords)
...
func getPostPobsList(combinedKeywords []string, searchParamsPreset *request.SeekSearchApiParams) []request.SeekSearchApiResponseData {
...
func batchPostDetailProcess(post *request.SeekSearchApiResponseData, searchParamsPreset *request.SeekSearchApiParams, readConfig *config.SearchConfig) *request.SeekPostDetails {
...
func fetchJobs(preset *request.SeekSearchApiParams, keyword string) ([]request.SeekSearchApiResponseData, error) {
	postData := []request.SeekSearchApiResponseData{}

	data, err := request.SeekSearchApiWithPreset(preset, keyword, 1)
	if err != nil {
		log.Println(err)
		return nil, err
	}
	...
	for pageNumber := 1; pageNumber <= pageTotal; pageNumber++ {
		...
		data, err := request.SeekSearchApiWithPreset(preset, keyword, pageNumber)
		if err != nil {
			log.Println(err)
		}
