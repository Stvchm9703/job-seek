...
func checkFetchUserAccountEmptyFields(req *protos.GetUserRequest) error {
	...
	if req.GetUserId() == "" && req.GetUserName() == "" && req.GetUserEmail() == "" && req.GetUserPhone() == "" {
		...
		return twirp.InvalidArgument.Error("reference field cannot be empty. Please provide at least one reference field, either userId, userName, userEmail or userPhone")
	}
