...
func OtherScoring(config *config.SearchConfig, post *request.SeekSearchApiResponseData, postDetail *request.SeekPostDetails) {
	calculateSalaryScore(config, post, postDetail)
	calculateCompanyFactorScore(config, post, postDetail)
}

func calculateSalaryScore(config *config.SearchConfig, post *request.SeekSearchApiResponseData, postDetail *request.SeekPostDetails) {
	score := 0
...
func calculateCompanyFactorScore(config *config.SearchConfig, post *request.SeekSearchApiResponseData, postDetail *request.SeekPostDetails) {
	...
	if strings.Contains(strings.ToLower(postDetail.CompanyDetails.Name), "recruit") ||
		...
		strings.Contains(strings.ToLower(postDetail.CompanyDetails.Specialties), "recruit") ||
		...
		strings.Contains(strings.ToLower(postDetail.CompanyDetails.Specialties), "agency") {
		tmpScore -= 5
